 ...
 *
 * A COPY of `apps/timeline-gstudio001/components/graph-view/playbar/playbar-styles.ts`,
 * which that app still uses. Keep the two in step by hand.
 *
 * DO NOT RE-RUN THE GENERATOR to refresh either one. The committed file carries
 * spacing edits made after generation, so the generator's output is OLDER than
 * what ships and would quietly undo them. The note below is the original's.
 *
