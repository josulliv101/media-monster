 ...
 * keeps every tile's call site untouched — but this one thing CSS cannot do. A
 * floating tooltip and a permanent label are different things to a screen
 * reader (`role="tooltip"` versus a plain label), and the description line is
 * DROPPED rather than hidden, so the choice has to be made in React.
 *
 * Defaults to false, which is the collapsed rail.
 */
export const RailLabelsInlineContext = createContext(false);

/**
 * A rail affordance's label. Shared by every tile so they cannot drift apart.
 *
 ...
 */
export function RailTooltipLabel({
  id,
  label,
  description,
}: Readonly<{ id: string; label: string; description?: string }>) {
  const inline = useContext(RailLabelsInlineContext);
  ...
  return (
    <span
      ...
      className={cn(
        ...
        "group-hover/rail-item:opacity-100 group-focus-visible/rail-item:opacity-100",
        ...
        // stamped by the rail on pointer clicks and cleared on pointerenter.
        "group-data-[tip-suppressed]/rail-item:invisible",
      )}
    >
