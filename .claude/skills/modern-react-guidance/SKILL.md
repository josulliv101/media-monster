...
> Installed from github.com/adhhamdev/modern-react-guidance @ 2ebb9d4 (MIT), with two local edits:
> the workflow no longer rewrites working code "on sight", and the closing "source of truth"
> line is gone. Where this repo's CLAUDE.md / AGENTS.md disagree, they win.
...
3. Don't introduce `forwardRef` / `useEffect`+fetch / hand-rolled form state machines in code you are writing or were asked to change. Don't rewrite existing working code for these patterns unless asked; mention them instead.
...
Update references when major React releases land.
